...
def progress(
...
):  # code for progress bar taken from https://blog.shichao.io/2012/10/04/progress_speed_indicator_for_urlretrieve_in_python.html
    global start_time
    if count == 0:
    ...
    sys.stdout.write(
        "\rProgress: %d%%, Downloaded: %d MB, Speed: %d KB/s"
...
def get_dbs_pvogs(db_folder):
    ...
    for i in urls:
        ...
        counter = counter + 1
    os.system(
        "cat {}/AllvogHMMprofiles/VOG*.hmm > {}/combined_AllvogHMMprofiles.hmm".format(
            db_folder, db_folder
        )
    )
...
def process_vogsTable_pvogs(db_folder):
    ...
    df_list = (
        df_list.groupby("vog")[["type", "phage_name", "description"]]
        .agg(lambda x: x.tolist())
        .reset_index()
    )
    df_list["description"] = df_list.vog.map(
        df_list.groupby("vog")["description"].sum().apply(np.unique)
    )
    df_list["type"] = df_list.vog.map(
        df_list.groupby("vog")["type"].sum().apply(np.unique)
    )
...
def process_vogsHMM_pvogs(db_folder):
    ...
    subprocess.run(["hmmpress", db_folder])
    end_time = "{:.2f}".format(time.time() - start_time)
    print("\n{:#^50}".format(" Done: " + end_time + " seconds "))
